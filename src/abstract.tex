...
La odometría visual-inercial (VIO) fusiona cámaras e IMUs para estimar la localización de robots y dispositivos de realidad virtual y aumentada (VR/AR) de forma precisa y robusta, y el VI-SLAM (Visual-Inertial SLAM) la complementa con un mapa persistente que habilita el cierre de ciclos y la corrección del error acumulado a largo plazo.
Estos sistemas deben operar en tiempo real sobre plataformas con recursos computacionales y energéticos estrictamente acotados, sin comprometer precisión ni robustez.
Los métodos de VIO del estado del arte logran rendimiento en tiempo real, pero acumulan \textit{drift} sin posibilidad de corrección global; los de VI-SLAM, por su parte, presentan dificultades para garantizar operación en tiempo real en escenarios típicos de robótica y VR/AR.
En este trabajo se presenta un sistema VI-SLAM eficiente que extiende BasaltMSD, una variante del algoritmo de VIO de Basalt adaptada a VR/AR con múltiples funcionalidades, incorporando \textit{loop closure} en tiempo real, optimización global de \textit{bundle adjustment} y eliminación de keyframes (\textit{culling}). La propuesta añade un módulo de loop closure eficiente basado en HashBoW con arquitectura asíncrona, que corrige el drift global sin degradar el rendimiento del front-end, y adapta la formulación de RootBA para soportar configuraciones multicámara con modelos de cámara arbitrarios, proveyendo un bundle adjustment numéricamente estable y ejecutable con precisión simple.
