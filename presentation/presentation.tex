...
% Mitad de una figura vertical: #1 es el recorte (trim) y #2 el archivo.
% Se dimensiona respecto de \paperwidth para poder usar todo el ancho de la pagina.
\newcommand{\halfimg}[2]{\includegraphics[trim=#1,clip,width=0.487\paperwidth,height=0.88\textheight,keepaspectratio]{#2}}
...
% Video a pantalla completa: diapositiva sin titulo ni margenes, la caja del
% video se escala hasta ocupar toda la pagina (respetando su relacion de aspecto).
% El clic reproduce en pdfpc / Acrobat; en otros visores se ve el frame poster.
\newcommand{\fullscreenmovie}[2]{%
  \begingroup
  ...
  \begin{columns}[c]
    \begin{column}{0.4\textwidth}
      ...
      \begin{itemize}
        ...
        \item \focusitem{4-5}{Cierre de ciclos}
      \end{itemize}
    \end{column}
    \begin{column}{0.6\textwidth}
      ...
      \only<2>{\movie[showcontrols,loop]{\includegraphics[height=8cm]{videos/front-end.poster.jpg}}{videos/front-end.webm}}%
      \only<3>{%
        \boxedimg{\linewidth}{4.8cm}{factor-graph.pdf}\\[0.25em]
        {\tiny Imagen extraída de Dellaert y Kaess, \emph{Factor Graphs for Robot Perception} (2017).}\\[0.7em]
      ...
      \only<4>{%
        \boxedimg{\linewidth}{6cm}{before-loop-closure.png}\\[0.25em]
        {\tiny Imagen extraída de Rady, \emph{Information-Theoretic Environment Modeling for Mobile Robot Localization} (2012).}%
      }%
      \only<5>{%
        \boxedimg{\linewidth}{6cm}{after-loop-closure.png}\\[0.25em]
        {\tiny Imagen extraída de Rady, \emph{Information-Theoretic Environment Modeling for Mobile Robot Localization} (2012).}%
      }%
    \end{column}
  \end{columns}
...
\begin{frame}[t]{1- Cierre de ciclos: detección}
  ...
  \only<2->{\\[0.5em]
    \boxedimg{\linewidth}{0.57\textheight}{hashbow.pdf}\\[0.25em]
    {\tiny Imagen extraída de Stricker, \emph{Efficient Techniques for Accurate Visual Place Recognition} (2020).}%
  }
\end{frame}
...
\begin{frame}{3- RootBA: idea clave}
  \begin{itemize}
    \item<1-> Para minimizar $E(x_p, x_l)$ hay que calcular
      los jacobianos $\mathbf{J}_p$ y $\mathbf{J}_l$:
      \[ \min_{\Delta\mathbf{x}_p,\,\Delta\mathbf{x}_l} \left\|
         \mathbf{r} + \begin{pmatrix} \mathbf{J}_p & \mathbf{J}_l \end{pmatrix}
         \begin{pmatrix} \Delta\mathbf{x}_p \\ \Delta\mathbf{x}_l \end{pmatrix} \right\|^2 \]
    \item<2-> RootBA marginaliza $\mathbf{J}_l$ mediante la factorización QR.
    \item<3-> Reduce el tamaño del problema.
    \item<4-> Evita el cómputo de la matriz hessiana $\mathbf{H} = \mathbf{J}^\top\mathbf{J}$.
  \end{itemize}
\end{frame}
...
%        \item<5-> Nuevo estado: $s=(s_b, s_l)$.
    ...
    \begin{column}{0.45\textwidth}
      ...
      {\scriptsize Ejemplo de un sistema multi-cámara.}\\[0.3em]
      {\tiny Imagen extraída de De Mayo, Cremers y Pire, \emph{The Monado SLAM Dataset} (IROS 2025).}
    \end{column}
  \end{columns}
...
\begin{frame}{Estudio de ablación}
  ...
  \begin{itemize}
    ...
    \item \textbf{BasaltLC}: cierre de ciclos y eliminación de keyframes.
    \item \textbf{BasaltLCR}: optimización global con RootBA.
  \end{itemize}
\end{frame}

\fullscreenmovie{videos/MPX.poster.jpg}{videos/MPX.webm}

\begin{frame}{Sistema propuesto vs. estado del arte en EuRoC y TUM-VI}
  \begin{table}[!htbp]
  \centering
  \caption{ATE [m] promedio por grupo de secuencias.}
...
\begin{frame}{Sistema propuesto vs. estado del arte en MSD}
  \begin{table}[!htbp]
  \centering
  \caption{ATE [cm], RTE [cm] y tasa de completitud (\%) promedio por casco.}
...
\begin{frame}{Conclusiones}
  \begin{itemize}
    \item<1-> VI-SLAM eficiente sobre BasaltMSD: cierre de ciclos, eliminación de keyframes y Bundle Adjustment global.
    \item<2-> Detección de ciclos con HashBoW y cadena de verificaciones robustas.
    \item<3-> Primera aplicación de RootBA como BA global en VI-SLAM: multicámara y precisión simple (float32).
    \item<4-> Mejor combinación de precisión, robustez y tiempo real en XR; supera a ORB-SLAM3 y OKVIS2.
    \item<5-> Se envió el trabajo a las Jornadas Argentinas de Robótica (JAR) 2026 y próximamente al ICRA.
    \item<6-> Cierre de ciclos usado en un trabajo del TUM CVG para el Hilti x Trimble SLAM Challenge 2026, quedando \#9.
    \note{Se esta usando en un sistema de Gaussian Splatting en real-time}
  \end{itemize}
\end{frame}

\begin{frame}{Trabajo futuro}
  \begin{itemize}
    \item<1-> BA online, integrado al mapa persistente.
    \item<2-> Términos de IMU / pose relativa en la optimización global.
    \item<3-> Verificación de ciclos más robusta (entornos difíciles).
%    \item<4-> Colaboración multiagente sobre un mapa compartido.
...
\begin{frame}{Rendimiento: tiempo real}
  \centering
  \boxedimg{0.8\linewidth}{0.72\textheight}{mean.timings.barplot-sp.pdf}\\[0.4em]
\end{frame}
...
\begin{frame}{Rendimiento: trayectorias extensas}
  \centering
  \boxedimg{0.8\linewidth}{0.72\textheight}{kftimings.MGO12-sp.pdf}\\[0.4em]
\end{frame}
...
\begin{frame}{Back-end: mapa persistente}
  ...
  \begin{columns}[c]
    \begin{column}{0.5\textwidth}
      \centering
      \boxedimg{\linewidth}{0.75\textheight}{persistent.map.png}
    \end{column}
    \begin{column}{0.5\textwidth}
      \centering
      \boxedimg{\linewidth}{0.75\textheight}{covisibility-sp.pdf}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{RootBA: precisión simple vs. doble (ATE)}
  \centering
  \footnotesize
  \begin{table}[!htbp]
  \centering
  \caption{ATE medio [cm]}
  \begin{tabular}{llrr}
  \toprule
...
\begin{frame}{Trayectorias estimadas en EuRoC y TUM-VI}
  % La figura original es vertical (4 secuencias x 2 paneles); se parte en dos
  % mitades (secuencias 1-2 y 3-4) que se ubican lado a lado para aprovechar el
  % ancho de la diapositiva (el \makebox permite exceder los margenes del texto).
  \vfill
  \makebox[\linewidth][c]{%
    \halfimg{0 729 0 0}{trajectories.ablation.euroc.tum.pdf}\hspace{0.01\paperwidth}%
    \halfimg{0 0 0 783}{trajectories.ablation.euroc.tum.pdf}}
  \vfill
  \note{El VIO (azul) acumula drift; con loop closure (verde) y BA global (amarillo) la trayectoria se alinea con el ground-truth.}
\end{frame}

\begin{frame}{Trayectorias estimadas en MSD}
  \vfill
  \makebox[\linewidth][c]{%
    \halfimg{0 720 0 0}{trajectories.ablation.msd.pdf}\hspace{0.01\paperwidth}%
    \halfimg{0 0 0 785}{trajectories.ablation.msd.pdf}}
  \vfill
  \note{En las secuencias de XR el drift de BasaltMSD es notorio; BasaltLC y BasaltLCR mantienen el error acotado.}
\end{frame}

\begin{frame}{Sistema propuesto vs. estado del arte: Trayectorias}
  \centering
  \boxedimg{\linewidth}{0.82\textheight}{sota.trajectories.pdf}
  \note{Magistrale 1: solo BasaltLCR detecta el ciclo al reingresar a la habitación. V102: BasaltLCR mejora al VIO pero no alcanza a OKVIS2/ORB-SLAM3. MIPP01: BasaltLCR es el único que mantiene el drift acotado; ORB-SLAM3 estima solo el 85\% de la secuencia.}
\end{frame}

\begin{frame}{Detecciones de ciclo en TUM-VI: Corridor y Magistrale}
  \vfill
  % Los graficos 3D son casi cuadrados: se dimensionan respecto de \paperwidth
  % (el \makebox permite exceder los margenes) para aprovechar la diapositiva.
  \makebox[\linewidth][c]{%
    \begin{minipage}[t]{0.48\paperwidth}
      \centering
      \boxedimg{\linewidth}{0.86\textheight}{TC1.loops-sp.pdf}\\[0.2em]
      {\scriptsize Corridor 1 (TC1)}
    \end{minipage}\hspace{0.02\paperwidth}%
    \begin{minipage}[t]{0.48\paperwidth}
      \centering
      \boxedimg{\linewidth}{0.86\textheight}{TM1.loops-sp.pdf}\\[0.2em]
      {\scriptsize Magistrale 1 (TM1)}
    \end{minipage}}
  \vfill
  \note{El eje vertical es el tiempo: cada línea une los dos instantes de la trayectoria que se conectan. En verde los ciclos aceptados, en rojo los detectados pero rechazados por la verificación.}
\end{frame}

\begin{frame}{Detecciones de ciclo en TUM-VI: Outdoors}
  \vfill
  % Los graficos 3D son casi cuadrados: se dimensionan respecto de \paperwidth
  % (el \makebox permite exceder los margenes) para aprovechar la diapositiva.
  \makebox[\linewidth][c]{%
    \begin{minipage}[t]{0.48\paperwidth}
      \centering
      \boxedimg{\linewidth}{0.86\textheight}{TO3.loops-sp.pdf}\\[0.2em]
      {\scriptsize Outdoors 3 (TO3)}
    \end{minipage}\hspace{0.02\paperwidth}%
    \begin{minipage}[t]{0.48\paperwidth}
      \centering
      \boxedimg{\linewidth}{0.86\textheight}{TO7.loops-sp.pdf}\\[0.2em]
      {\scriptsize Outdoors 7 (TO7)}
    \end{minipage}}
  \vfill
  \note{En exteriores los ciclos se concentran al inicio y al final, cuando se vuelve al punto de partida.}
\end{frame}

\begin{frame}{Comunicación entre \texttt{LoopClosing} y \texttt{MapDatabase}}
  \centering
  % Diagramas verticales: la altura es la que limita, sobra ancho en la diapositiva.
  \begin{columns}[t]
    \begin{column}{0.45\textwidth}
      \centering
      \boxedimg{\linewidth}{0.76\textheight}{loop_decision_ok-sp.pdf}\\[0.3em]
      {\scriptsize Detección con suficiente drift}
    \end{column}
    \begin{column}{0.45\textwidth}
      \centering
      \boxedimg{\linewidth}{0.76\textheight}{loop_decision_rej-sp.pdf}\\[0.3em]
      {\scriptsize Detección con drift insuficiente}
    \end{column}
  \end{columns}
  \note{Ante un ciclo, MapDatabase evalúa el drift acumulado. Si amerita corrección, envía a LoopClosing una copia liviana del mapa y las aristas del grafo para el PGO, y luego aplica las poses optimizadas. Si no, le avisa que la optimización no procede. En ambos casos el proceso termina con la fusión de mapa.}
\end{frame}

\begin{frame}{Búsqueda de correspondencias por reproyección}
  \centering
  \boxedimg{\linewidth}{0.8\textheight}{reprojection_illustration-sp.pdf}
  \note{Los landmarks de la isla de candidatos que quedaron sin emparejar se reproyectan sobre el keyframe más reciente usando la pose de la verificación geométrica previa. En una ventana de 16 px alrededor de cada proyección se detectan keypoints con FAST y se computan descriptores ORB; si alguno coincide con el del landmark, la correspondencia se suma a la verificación siguiente.}
\end{frame}

\begin{frame}{Detección de ciclos: recuperación de candidatos con BoW}
  \centering
  \vfill
  \boxedimg{\linewidth}{0.5\textheight}{bow-process.pdf}
  \vfill
\end{frame}
